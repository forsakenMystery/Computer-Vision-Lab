...
I2 = cv2.imread('C:\\Users\\Hamed Khashehchi\\Desktop\\cv-lab12\\scene.jpg')
G2 = cv2.cvtColor(I2, cv2.COLOR_BGR2GRAY)
keypoints2, desc2 = sift.detectAndCompute(G2, None);  # opencv 3

fnames = glob.glob('C:\\Users\\Hamed Khashehchi\\Desktop\\cv-lab12\\obj?.jpg')
fnames.sort()
for fname in fnames:
    ...
    pts = np.float32([[0, 0],
                      [0, height],
                      [width, height],
                      [width, 0]]).reshape(-1, 1, 2)  # this needs to be changed
    J = cv2.warpPerspective(pts, H, (I2.shape[1], I2.shape[0]))
